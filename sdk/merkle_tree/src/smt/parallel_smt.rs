...
use crate::smt::storage::SMTStorage;
use crate::Fr;
...
#[derive(Debug)]
pub struct SparseMerkleTree<T, Hash, H, S>
where
    T: GetBits,
    Hash: Clone + Debug,
    H: Hasher<Hash>,
...
impl<T, Hash, H, S> Clone for SparseMerkleTree<T, Hash, H, S>
where
    T: GetBits + Clone,
    Hash: Clone + Debug,
    H: Hasher<Hash> + Clone,
    ...
    S: SMTStorage<ItemIndex, T>,
{
    ...
    pub fn verify_proof(&self, element_index: u64, element: T, proof: Vec<(Hash, bool)>) -> bool {
        let mut proof_index: u64 = 0_u64;
    ...
    S: SMTStorage<ItemIndex, T>,
{
    ...
    pub fn insert(&mut self, item_index: u64, item: T) {
        ...
        self.storage.insert(item_index, item);
    ...
    pub fn remove(&mut self, index: u64) -> Option<T> {
        ...
        let old = self.storage.remove(&index);
        let item = T::default();

        self.insert(index as u64, item);
    ...
    /// the direction.
    pub fn merkle_path(&self, index: u64) -> Vec<(Hash, bool)> {
    ...
    fn get_hash(&self, node_ref: NodeRef) -> (Hash, Vec<(NodeIndex, Hash)>) {
        ...
        let (hash, mut updates) = {
            if node.depth == self.tree_depth {
                ...
                let item_bits = self.storage.get(&item_index).unwrap().get_bits_le();
...
impl<T, H, S> SparseMerkleTree<T, Fr, H, S>
where
    T: GetBits,
    H: Hasher<Fr>,
    S: SMTStorage<ItemIndex, T>,
...
mod tests {
    ...
    type TestSMT = SparseMerkleTree<TestLeaf, u64, TestHasher, DefaultMemorySMTStorage<TestLeaf>>;
    ...
    fn test_merkle_tree_insert() {
        let mut tree = TestSMT::new(64);

        assert_eq!(tree.capacity(), 18446744073709551616);

        tree.insert(0, TestLeaf(1));
        assert_eq!(tree.root_hash(), 952046316);

        tree.insert(0, TestLeaf(2));
        assert_eq!(tree.root_hash(), 928864106);

        tree.insert(3, TestLeaf(2));
        assert_eq!(tree.root_hash(), 635900515);
    }
    ...
    fn merkle_path_test() {
        ...
        for &(idx, value) in &test_vector[..] {
            ...
            assert_eq!(proof_index, idx as u128);
